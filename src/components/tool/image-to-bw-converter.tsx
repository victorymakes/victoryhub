      ...
      return new Promise((resolve, reject) => {
        ...
        reader.onload = (event) => {
          ...
          img.onload = () => {
            ...
            const outputFormat = getImageMimeType(imageItem.originalFormat);
  ...
  const downloadAll = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    await Promise.all(
      completedImages.map(async (img) => {
        ...
        return zip.file(getConvertedImageFileName(img), blob);
      }),
    );
                ...
                <div className="bg-background rounded-lg shadow-lg max-w-4xl w-full max-h-[90vh] flex flex-col">
                  <div className="flex items-center justify-between p-4 border-b">
                    <h3 className="text-lg font-medium">
                      {images[previewIndex]?.file.name || t("preview")}
                    </h3>
                    <Button
                      variant="ghost"
                      size="icon"
                  ...
                  <div className="flex-1 overflow-hidden relative p-4 flex items-center justify-center">
                    {images[previewIndex]?.status === "done" &&
                    images[previewIndex]?.convertedUrl ? (
                      <Image
                        src={images[previewIndex].convertedUrl}
                        alt={images[previewIndex].file.name}
                        width={800}
                        height={600}
                        className="object-contain max-h-full"
                      />
                    ) : images[previewIndex]?.status === "error" ? (
                      <div className="flex flex-col items-center justify-center h-full gap-2">
                        <AlertTriangle className="h-8 w-8 text-destructive" />
                        <p className="text-sm text-destructive">
                          {images[previewIndex]?.error ||
                            t("errors.conversionFailed")}
                        </p>
                        <Image
                          src={getOriginalImageUrl(images[previewIndex].file)}
                          alt={images[previewIndex].file.name}
                          width={800}
                          height={600}
                  ...
                  <div className="p-4 border-t flex justify-between items-center">
                    ...
                    {images[previewIndex]?.status === "done" && (
                      <Button
                        variant="default"
                        size="sm"
                        onClick={() => downloadSingleFile(images[previewIndex])}
                      >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {img.status === "error" && (
                            <div className="flex items-center gap-1 text-xs text-destructive">
                              ...
                              <span>
                                {img.error || t("errors.conversionFailed")}
                              </span>
                            </div>
                          )}
                          {img.status === "done" && (
