...
import { AlertTriangle, Copy, ExternalLink, Info, Trash2 } from "lucide-react";
      ...
      <Card>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            <label htmlFor="tracker-input" className="text-sm font-medium">
              {t("inputLabel")}
            </label>
            <textarea
              id="tracker-input"
          ...
          {trackers.length > 0 && (
            <div className="space-y-3">
              <h3 className="text-sm font-medium">{t("trackersFound")}</h3>
              <div className="p-3 border rounded-lg bg-muted/30">
                <div className="flex flex-wrap gap-2">
                  {trackers.map((tracker) => (
                    <Badge
                      key={`${tracker.name}=${tracker.value}`}
                      variant="outline"
                      className="flex items-center gap-1"
                    >
                      <span className="font-mono">{tracker.name}</span>
          ...
          {cleanedUrl && (
            <div className="space-y-3">
              <h3 className="text-sm font-medium">{t("cleanedUrl")}</h3>
