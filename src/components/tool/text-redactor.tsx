...
import { AlertTriangle, Check, Copy, Settings2, Text } from "lucide-react";
      ...
      redactionOptions.forEach((option) => {
        if (option.enabled) {
          if (maskingType === "full") {
            ...
            processedText = processedText.replace(option.pattern, (match) =>
              redactionChar.repeat(match.length),
            );
          } else {
          ...
          <Card>
            <CardContent className="pt-6 space-y-6">
              ...
              <div className="space-y-3">
                <label htmlFor="redactor-input" className="text-sm font-medium">
                  {t("input")}
                </label>
                <textarea
                  id="redactor-input"
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={processRedaction} className="flex-1">
                  {t("redactButton")}
                </Button>
              </div>
            </CardContent>
        ...
        <TabsContent value="settings" className="mt-4">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <h3 className="text-lg font-semibold">{t("redactionOptions")}</h3>
            </CardHeader>
            <CardContent className="space-y-6">
              {/* Masking Type */}
              <div className="space-y-3">
                <p className="text-sm font-medium">{t("maskingType")}</p>
                <div className="flex gap-2">
                  <Button
                    variant={maskingType === "full" ? "default" : "outline"}
                    size="sm"
                  ...
                  <Button
                    variant={maskingType === "partial" ? "default" : "outline"}
                    size="sm"
                    onClick={() => setMaskingType("partial")}
                    className="flex-1"
                  >
                    {t("partialMask")}
                  </Button>
              ...
              {/* Redaction Character */}
              <div className="space-y-3">
                <p className="text-sm font-medium">{t("redactionCharacter")}</p>
                <div className="flex gap-2">
                  {["x", "*", "#", "■"].map((char) => (
                    <Button
                      key={char}
                      variant={redactionChar === char ? "default" : "outline"}
                      size="sm"
                      onClick={() => setRedactionChar(char)}
              ...
              {/* Redaction Options */}
              <div className="space-y-3">
                <p className="text-sm font-medium">{t("dataTypesToRedact")}</p>
              ...
              <div className="space-y-3">
                <label
                  htmlFor="redactor-custom-pattern"
                  className="text-sm font-medium"
                >
                  {t("customPattern")}
                </label>
                <input
                  id="redactor-custom-pattern"
                  type="text"
                  value={customPattern}
