...
`);
  ...
  const generateFavicon = useCallback(
    (inputText: string, size: FaviconSize): Map<FaviconFormat, string> => {
      ...
      const scaledFontSize = Math.max(8, Math.floor(fontSize * (size / 64)));
      ...
      // 绘制文本：往下移一点
      ctx.fillText(inputText, canvas.width / 2, canvas.height / 2 + offset);
  ...
  const generateFavicons = useCallback(() => {
    ...
    if (isBatchMode) {
      ...
      if (lines.length > 1) {
        setWarning(t("batchModePreviewWarning", { count: lines.length }));
      }
    } else {
      if (!text.trim()) {
    ...
    try {
      ...
      for (const currentText of textsToProcess) {
        const textFavicons = new Map<FaviconSize, Map<FaviconFormat, string>>();
      ...
      <Card>
        <CardContent className="space-y-6">
          ...
          {!isBatchMode ? (
            ...
            <div className="space-y-2">
              <Label htmlFor="batch-texts">{t("batchTexts")}</Label>
              <Textarea
                id="batch-texts"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={shape}
                onValueChange={(value) => setShape(value as FaviconShape)}
              >
          ...
          {generatedFavicons.size > 0 && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-h-80 overflow-y-auto p-2">
                {Array.from(generatedFavicons.entries()).flatMap(
                  ([text, textFavicons], textIndex) => {
                    if (isBatchMode && textIndex > 0) {
                      const sizeFormats = textFavicons.get(16);
                      if (!sizeFormats) return null;

                      const pngUrl = sizeFormats.get("png");
                    ...
                    return Array.from(textFavicons.entries()).map(
                      ([size, formats]) => {
                        const pngUrl = formats.get("png");
                        if (!pngUrl) return null;
